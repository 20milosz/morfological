...
int8_t convertBitmapToBinaryImage(bitmap* bitmap_in, Matrix* mat_out) {
	...
	height = (height + blockD - 1) / blockD + strucElDim - 1;
	width = (width + blockD - 1) / blockD + strucElDim - 1;
	createHostMatrix(mat_out, height, width, height*width * sizeof(int));
	printf("wysokosc%d\n", height);
	verifyHostAllocation(*mat_out);
	for (int i = 0; i < (bitmap_in->hp->width)*(bitmap_in->hp->height); i++)
	{
	...
	for (i = 0; i < bitmap_in->hp->width; i++)
	{
		...
		{
			idx = i+1 + bitmap_in->hp->width*(j+1);
			if (bitmap_in->image[idx].r < 170) {
				mat_out->elements[idx] = 1;
			}
			else {
				mat_out->elements[idx] = 0;
			}
